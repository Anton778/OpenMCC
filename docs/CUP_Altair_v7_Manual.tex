...
\hypersetup{colorlinks=true,linkcolor=AltairBlue,urlcolor=AltairBlue,pdftitle={ЦУП Альтаир v7 — руководство пользователя}}
\pagestyle{fancy}
\fancyhf{}
\lhead{ЦУП Альтаир v7}
...
\begin{center}
{\Huge\bfseries\sffamily ЦУП Альтаир v7}\\[2mm]
...
\textbf{Проверенная конфигурация v7.} На стенде устойчивый приём получен при широкой полосе приёмника \textbf{203 кГц}. Поэтому в v7 именно эта полоса установлена по умолчанию вместо 58 кГц.
...
\item \code{CUP-Altair-Setup-0.7.0.exe} — программа ЦУП Альтаир для Windows.
...
\textbf{STM32 + CC1101} \quad $\longrightarrow$ \quad \textbf{435.000 МГц} \quad $\longrightarrow$ \quad \textbf{CC1101 + ESP32-WROOM-32} \quad $\longrightarrow$ \quad \textbf{USB 115200} \quad $\longrightarrow$ \quad \textbf{ЦУП Альтаир v7}
}}
\end{center}
...
В минимальном 29-символьном пакете состояния антенны пока нет. Поэтому ЦУП v7 показывает в этой карточке \textbf{«Н/Д»}, а не прочерк. Когда в будущий формат будет добавлено поле антенны, значения должны трактоваться как \code{1 = РАСКРЫТА}, \code{0 = СЛОЖЕНА}.
...
Установщик v7 содержит скомпилированную прошивку радиошлюза. В разделе прошивки ESP32 можно записать её без Arduino IDE. После успешной прошивки перейти к подключению основного порта.

\textbf{Не следует} после ручной загрузки тестового скетча нажимать «Прошить ESP32» старой версией программы: это перезапишет рабочую прошивку другой сборкой. Для v7 встроенная прошивка уже использует полосу 203 кГц.

\section{Запуск ЦУП Альтаир}

\begin{enumerate}[leftmargin=7mm]
\item Установить \code{CUP-Altair-Setup-0.7.0.exe}.
...
В Serial Monitor есть \code{PACKET RECEIVED}, а ЦУП пустой & ESP32 должна выдавать нормализованную строку \code{$TEL,...}; закрыть Serial Monitor и открыть тот же COM-порт в ЦУПе.\\
...
\vfill
\begin{center}
\small ЦУП Альтаир v7 · Технопром 2026 · Миссия на Луну
\end{center}
