...
В прошивке ESP32 версии **8.3.0** приём CC1101 работает неблокирующим способом по прерыванию `GDO0`. Поэтому USB-команды обрабатываются постоянно и передача начинается сразу после нажатия кнопки в ЦУПе, без ожидания тайм-аута приёмника. В строку подтверждения `$ACK,RF_TX` добавлено поле `TX_MS` с фактической длительностью радиопередачи.

Частоту, мощность TX и полосу RX можно изменять из панели команд ЦУПа. Команда `$CMD,RADIO,RESET` возвращает исходные значения `435.000 МГц`, `5 дБм` и `203 кГц`. Настройки хранятся до перезапуска ESP32; после включения питания используется исходный профиль.
