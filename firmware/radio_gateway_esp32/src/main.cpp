...
/*
 * OpenMCC Radio Gateway
 * Firmware version: 0.1.0
 * Target: ESP32-WROOM-32 / generic ESP32 DevKit
 *
 ...
 *   $CMD,RADIO_OFF
 *
 * Any other $CMD,... line is treated as a spacecraft command and is
 * forwarded over the selected RF module. Incoming RF text is forwarded
 * unchanged to USB, so $TEL/$ACK/$ERR/$INFO packets remain compatible
 * with the existing OpenMCC parser.
 */
...
// CC1101 wiring used by OpenMCC v0.3 UI.
...
// E32-433T30D wiring used by OpenMCC v0.3 UI.
...
bool validCcBandwidth(float value) {
  ...
  for (float item : allowed) {
    if (nearlyEqual(value, item)) return true;
  }
  return false;
}
...
void loadStoredConfig() {
  ...
  if (activeType != RadioType::CC1101 && activeType != RadioType::E32) {
    activeType = RadioType::None;
  }

  ccConfig.frequency = preferences.getFloat("ccFreq", 435.000f);
  ccConfig.power = preferences.getChar("ccPower", 10);
  ccConfig.bitRate = preferences.getFloat("ccRate", 38.4f);
  ccConfig.modulation = preferences.getString("ccMod", "GFSK");
  ccConfig.bandwidthAuto = preferences.getBool("ccBwAuto", true);
...
bool initializeCc1101(bool verbose = true) {
  ...
  if (ccConfig.modulation == "MSK") {
    if (verbose) sendError("RADIO_CONFIG,CC1101_MSK_NOT_IMPLEMENTED");
    return false;
  }
  ...
  radioReady = true;
  if (verbose) {
    sendInfo("RADIO_READY,TYPE=CC1101,CHIP=" + String(cc1101.getChipVersion()));
  }
  return true;
}
...
bool configureE32Hardware(uint32_t oldBaud, bool verbose = true) {
  ...
  option |= 0x40; // push-pull UART output
...
void sendRadioStatus() {
  String prefix = "RADIO,TYPE=" + radioTypeName(activeType) +
                  ",READY=" + String(radioReady ? 1 : 0);
...
bool applyCcCommand(const String &line) {
  ...
  if (parseParameter(line, "CRC", value)) {
    if (!parseBoolValue(value, candidate.crc)) {
      sendError("RADIO_CONFIG,CRC=0|1");
      return false;
    }
  }
  if (parseParameter(line, "CCA", value)) {
    if (!parseBoolValue(value, candidate.cca)) {
...
bool applyE32Command(const String &line) {
  ...
  if (parseParameter(line, "FEC", value)) {
    if (!parseBoolValue(value, candidate.fec)) {
...
void forwardToRadio(const String &line) {
  ...
  if (ok) {
    sendAck("RADIO_TX,TYPE=" + radioTypeName(activeType) + ",BYTES=" + String(line.length()));
  }
}
...
void processUsbLine(String line) {
  ...
  if (upper == "$CMD,GATEWAY_INFO") {
    sendInfo("GATEWAY=OPENMCC,DEVICE=ESP32-WROOM-32,FW=" + String(FW_VERSION));
    return;
  }
...
void pollCc1101() {
  ...
  if (state == RADIOLIB_ERR_NONE) {
    payload.trim();
    if (!payload.isEmpty()) Serial.println(payload);
...
void setup() {
  ...
  preferences.begin("openmcc-rf", false);
  loadStoredConfig();

  sendInfo("GATEWAY=OPENMCC,DEVICE=ESP32-WROOM-32,FW=" + String(FW_VERSION) + ",BOOT=1");
