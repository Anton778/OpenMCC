...
7. После подключения блок соединения можно свернуть.
...
Широкая полоса **203 кГц** выбрана намеренно: на стенде некоторые модули имели заметный частотный уход относительно 435.000 МГц.
...
где `ANTENNA=1` — раскрыта, `ANTENNA=0` — сложена.
...
на каждом графике будут отдельные линии:

- `ID 01`
- `ID 02`
- `ID 07`

Цвет закрепляется за ID на текущую сессию. Кнопка **«Сбросить принятые данные»** очищает карточки, графики, счётчики и сырой COM-журнал.
...
> Бортовая прошивка спутника должна отдельно реализовать реакцию на эти команды. Даже без неё передачу ЦУПа можно сразу увидеть на SDR и проверить, что CC1101 подключён правильно.
...
Причины редкого обновления обычно находятся ниже по тракту:

- реальный период передачи конкретного спутника;
- коллизии, если несколько аппаратов передают одновременно на одной частоте;
- частотный уход передатчика;
- неправильный COM-порт или занятый Serial Monitor;
- потеря радиопакета по аппаратному CRC.
...
- отдельные линии графиков для каждого `ID`;
- сырой COM-терминал;
- окно **«О программе»**;
- новый значок Альтаира;
