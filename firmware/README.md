...
В шаге 02 номер пакета последовательно изменяется от `00001` до `99999`. Остальные телеметрические параметры остаются постоянными. Поле `CHECKSUM` пересчитывается автоматически, поскольку оно зависит от изменившегося текста пакета. Приёмник и обработка команд в обоих шагах отсутствуют.

Следующие учебные варианты будут добавляться как шаги 03, 04 и далее без изменения предыдущих примеров.
