...
/*
 * ЦУП Альтаир — спутниковый скетч, шаг 03.
 *
 * Отличие от шага 02:
 *   - поле UPTIME теперь содержит реальное время работы в секундах;
 *   - значение вычисляется функцией millis() с момента включения Arduino Nano.
 *
 * Уже реализовано на предыдущем шаге:
 *   - поле PACKET увеличивается после каждой попытки передачи;
 *   - CHECKSUM пересчитывается автоматически.
 *
 * Поля ID, PANEL_POWER, VOLT, MCU_TEMP и MODE пока остаются постоянными.
...
void buildTelemetryPacket(char* packet, size_t packetSize) {       // Формирует пакет с текущими PACKET и UPTIME.
  ...
  snprintf(                                                         // Записывает изменяемые и постоянные поля.
    ...
    "02,%05lu,%05lu,1.00,4.20,31.6,1,",                            // Оставляет изменяемыми PACKET и UPTIME.
...
void setup() {                                                                  // Выполняется один раз после включения Arduino Nano.
  ...
  Serial.println(F("Шаг 03: передатчик со счётчиком пакетов и UPTIME готов"));  // Сообщает об успешном запуске.
}

void loop() {                                                                   // Выполняет очередной цикл передачи.
  char telemetryPacket[48];                                                     // Выделяет память для полного текстового пакета.
  buildTelemetryPacket(telemetryPacket, sizeof(telemetryPacket));               // Формирует строку с актуальными PACKET и UPTIME.
