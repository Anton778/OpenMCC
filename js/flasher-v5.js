...
/* ============================================================
   ЦУП Альтаир — integrated ESP32 radio-gateway flasher
   Release v5 / 0.5.0
   ============================================================ */
...
function uiLog(message) {
    ...
    if (!text) return;
    el.log.textContent += `[${new Date().toISOString().slice(11, 23)}] ${text}\n`;
...
function addCss() {
    if (document.querySelector("link[data-altair-flasher]")) return;
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "/css/flasher.css";
    link.dataset.altairFlasher = "1";
...
function addNav() {
    ...
    button.dataset.tip = "Прошивка ESP32-WROOM-32 радиошлюза прямо из ЦУП Альтаир.";
...
function addHelp() {
    ...
    if (!body || body.querySelector("[data-altair-flasher-help]")) return;
    const section = document.createElement("section");
    section.dataset.altairFlasherHelp = "1";
    section.innerHTML = `
        <h3>Прошивка ESP32 радиошлюза</h3>
        <p>Установленная версия ЦУП Альтаир содержит готовый бинарный комплект ESP32-WROOM-32. Arduino IDE не требуется: выберите плату, запустите прошивку, дождитесь проверки MD5 и затем нажмите «Подключить как шлюз».</p>`;
...
function addPanel() {
    ...
    panel.innerHTML = `
        ...
        <p class="panelLead">Встроенная запись прошивки радиошлюза ЦУП Альтаир в ESP32-WROOM-32 без Arduino IDE и PlatformIO.</p>
        <div class="flasherIntro">
            <div>
                <div class="flasherSummary">
                    <div><span>Целевая плата</span><strong>ESP32-WROOM-32</strong></div>
                    <div><span>Прошивка шлюза</span><strong id="firmwareVersion">загрузка…</strong></div>
            ...
            <div class="flasherControls">
                ...
                <div class="flasherSafety"><strong>Без полного стирания.</strong> Записываются bootloader, таблица разделов, boot_app0 и приложение. NVS целиком не стирается.</div>
                <div class="flasherActions">
                    <button type="button" id="firmwareFlashButton" class="flasherPrimary" disabled data-tip="Выберите ESP32-WROOM-32. Перед записью ЦУП проверит семейство чипа.">Прошить радиошлюз</button>
                    <button type="button" id="firmwareReconnectButton" class="flasherSecondary" disabled data-tip="После успешной записи подключить ESP32 к основному порту и запросить состояние шлюза.">Подключить как шлюз</button>
                </div>
            </div>
        </div>
...
function validateManifest(manifest) {
    if (!manifest || String(manifest.targetChip).toUpperCase() !== "ESP32") {
        throw new Error("Некорректный манифест прошивки");
    }
    if (!Array.isArray(manifest.images) || manifest.images.length !== 4) {
        throw new Error("Комплект прошивки должен содержать четыре образа");
    }
...
async function prepare() {
    if (!navigator.serial) throw new Error("Web Serial API недоступен");
    status("ПОДГОТОВКА", "warning");
    stage("Загрузка Espressif esptool-js…");
    uiLog(`ЦУП Альтаир flasher v${VERSION}`);
    ...
    if (!response.ok) throw new Error("Встроенная прошивка отсутствует. Установите полный релиз ЦУП Альтаир v5.");
    ...
    state.manifest.images.forEach(image => {
        uiLog(`${image.label}: 0x${Number(image.address).toString(16)} · ${image.size} B · ${image.md5}`);
    });
    projectLog("Встроенный прошивальщик ESP32 готов", "success", { firmwareVersion: state.manifest.firmwareVersion });
}

async function loadImages() {
    const result = [];
    for (const image of state.manifest.images) {
        stage(`Чтение встроенного образа: ${image.label}…`);
        ...
        if (data.byteLength !== Number(image.size)) throw new Error(`Размер ${image.file} не совпадает с манифестом`);
...
function assertClassicEsp32(description) {
    ...
    if (family !== "ESP32") {
        throw new Error(`Обнаружено семейство ${family || description || "UNKNOWN"}. Эта прошивка предназначена только для классического ESP32 / ESP32-WROOM-32.`);
    }
}
...
async function verify(images) {
    ...
    uiLog("Проверка MD5 записанных областей Flash…");
    for (let index = 0; index < images.length; index += 1) {
        ...
        progress(82 + ((index + 1) / images.length) * 13);
    }
}
...
async function flash() {
    ...
    try {
        ...
        if (current?.connected || current?.connecting) {
            ...
            uiLog("Основной Serial отключается перед прошивкой.");
        ...
        const offsets = images.map((_, index) => images.slice(0, index).reduce((sum, item) => sum + item.data.byteLength, 0));
        ...
        await state.loader.writeFlash({
            ...
            reportProgress(fileIndex, written, total) {
                ...
                const ratio = totalBytes > 0
                    ? (offsets[fileIndex] + Math.min(item.data.byteLength, part)) / totalBytes
                    : 0;
                progress(8 + ratio * 72);
        ...
        projectLog("ESP32 радиошлюз прошит из ЦУП Альтаир", "success", { firmwareVersion: state.manifest.firmwareVersion });
    } catch (error) {
        status("ОШИБКА", "error");
...
async function reconnect() {
    ...
    try {
        ...
        uiLog("Радиошлюз подключён к ЦУП Альтаир на 115200 бод.");
        await window.OpenMCCSerial.sendCommand("GATEWAY_INFO");
        await new Promise(resolve => setTimeout(resolve, 120));
        await window.OpenMCCSerial.sendCommand("RADIO_STATUS");
    } catch (error) {
        uiLog(`Ошибка подключения шлюза: ${error?.message || error}`);
        projectLog(`Не удалось подключить прошитый шлюз: ${error?.message || error}`, "error");
    }
}
