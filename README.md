...
**ЦУП «Альтаир»** — учебный центр управления полётами, разработанный для сопровождения CubeSat-проектов.
Функциональные возможности:
Приём и визуализация телеметрии (отображение параметров и графиков для нескольких аппаратов одновременно).
Радиопередача команд на спутник.
В перспективе: интеграция и управление поворотным устройством антенны.

ЦУП адаптирован для работы в связке с образовательными конструкторами CubeSat от компании IntroSat. Бортовая система этих наборов включает микроконтроллер STM32 (Blue Pill) и фирменные радиомодули на базе чипа CC1101.
...
Широкая полоса приёмника ЦУПа **203 кГц** выбрана намеренно: на стенде некоторые модули имели заметный частотный уход относительно 435.000 МГц. Этот набор значений используется как исходный профиль при включении ESP32. В панели команд можно изменить частоту, мощность TX и полосу приёмника ЦУПа, а кнопка **«Сбросить к исходным»** возвращает `435.000 МГц`, `5 дБм` и `203 кГц`.

Доступные настройки CC1101:

- частота наземного модуля: от `387.000` до `464.000 МГц`;
- мощность TX: `−30`, `−20`, `−15`, `−10`, `0`, `5`, `7` или `10 дБм`;
- полоса приёмника ЦУПа: `58`, `68`, `81`, `102`, `116`, `135`, `162`, `203`, `232`, `270`, `325`, `406`, `464`, `541`, `650` или `812 кГц`.

Настройки действуют до перезапуска ESP32. После перезапуска автоматически восстанавливается исходный профиль. При изменении частоты бортовой передатчик необходимо настроить на ту же частоту.
...
Базовый пакет с температурой бортового компьютера:
...
`MCU_TEMP` — ориентировочная температура кристалла бортового микроконтроллера в градусах Цельсия. Поле располагается непосредственно после напряжения аккумулятора. Для точных абсолютных измерений внутренний датчик конкретной платы необходимо откалибровать.

Поддерживается и расширенный вариант с состоянием раскрытия антенны:

```text
02,00001,00015,1.00,4.20,31.6,1,1,1A
```
...
Для совместимости ЦУП продолжает принимать прежние семипольные пакеты без температуры.
...
Служебная команда `$CMD,RADIO_STATUS` возвращает текущий профиль наземного шлюза. Интерфейс использует следующие команды:

```text
$CMD,RADIO,SET,FREQ=433.920,POWER=5,BW=203
$CMD,RADIO,RESET
```

Первая команда применяет выбранные параметры, вторая возвращает исходный профиль. ESP32 проверяет допустимость значений и при ошибке восстановления сохраняет предыдущие рабочие настройки.
